...
public class BookService {
	...
	public boolean isBookInStock(Book b) {
		 ...
		 * In the delta, we will check if the book exists by checking the book name and
		 * book author
		 * 
		 */
		List<Book> checkedOutBooks = checkoutService.getCheckOuts().stream().map(ch -> ch.getBook())
		...
		Optional<Book> inStock = delta.stream()
				.filter(d -> d.getAuthor().equals(b.getAuthor()) && d.getName().equals(b.getName())).findFirst();
	...
	public void addBooks(List<Book> newBooks) {
		...
		newBooks.stream().forEach(t -> {
			Optional<Book> eb = allBooks.stream()
					.filter(a -> a.getAuthor().equals(t.getAuthor()) && a.getName().equals(t.getAuthor())).findFirst();
			Book newBook = null;
			if (eb.isPresent()) {
